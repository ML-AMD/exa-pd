import sys
import pytest
from tools.logging_config import ExaPdLogger, exapd_logger
...
def test_stdout_stderr(capsys):
    ...
    logger = ExaPdLogger(level_name="DEBUG", logger_name=log_name)
...
def test_global_logger(capsys):
    ...
    check_output(capsys, exapd_logger, "info", on_stdout=True)
    check_output(capsys, exapd_logger, "warning", on_stdout=False)
    check_output(capsys, exapd_logger, "error", on_stdout=False)
    # should not generate an output
    exapd_logger.debug("should_not_trigger_an_output")
    out, err = capsys.readouterr()
    assert "" == out
    assert "" == err
