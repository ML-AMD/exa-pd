...
if __name__ == '__main__':
    ...
    for job in jobs:
        if os.path.exists(f"{job._dir}/DONE"):
            continue
        exapd_logger.info(f"Submitted {job._dir}, {job._arch}, priority: {job._priority}")
        ...
        if job._arch == "cpu":
            try:
                ...
                exapd_logger.warning(f"{e}: Launching cpu job failed: {job._dir}")
        if job._arch == "gpu":
            try:
                ...
                exapd_logger.warning(f"{e}: Launching gpu job failed: {job._dir}")
