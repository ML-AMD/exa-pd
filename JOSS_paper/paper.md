...
![Strong scaling of exa-PD workflow on NERSC’s Perlmutter supercomputer. Wall-clock times are shown for both GPU and CPU nodes.](computer_time.png)

Figure 3 summarizes the strong scaling results, evaluated on both GPU and CPU nodes. The benchmark tests are performed on Perlmutter at the National Energy Research Scientific Computing Center (NERSC). The execution time is plotted as a function of the number of compute nodes, along with the ideal linear scaling behavior (dashed lines) for reference. The GPU results closely follow the ideal trend up to 32 nodes, achieving ~95% parallel efficiency. The CPU results exhibit good scaling (~75% efficiency), with increasing deviation at larger node counts, likely due to scheduling overhead and I/O contention.
